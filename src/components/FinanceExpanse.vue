...
const expanse = async () => {
  try {
    ...
    const { data } = await api.post("/fund/expanse", payload, {
  ...
  <form @submit.prevent="expanse" class="flex flex-col gap-3 items-start lg:justify-center mb-10 pt-5 px-5 w-full">
    <div class="w-full">
      <div class="grid gap-2 w-full">
        ...
        <Input id="nominalTopup" v-model="nominal" placeholder="Jumlah Pengeluaran" type="tel" class="w-full text-sm" required />
      </div>
      <!-- Nama Produk -->
